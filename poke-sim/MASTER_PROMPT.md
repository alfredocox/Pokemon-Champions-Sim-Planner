...
│   ├── data.js                     ← BASE_STATS, POKEMON_TYPES_DB (500+ mons), TEAMS (27 teams)
...
│   │   ├── seed_teams_v2.sql       ← Generated 27-team fresh-DB/reference seed
...
Supabase project `ymlahqnshgiarpbgxehp` (us-west-2, Postgres 17.6, ACTIVE_HEALTHY) is up. Schema, RLS, adapter wiring, and analysis persistence are active. Current canonical seed target is 27 teams; existing live DBs with analysis history should use `poke-sim/db/migrations/2026_06_20_align_shared_27_team_catalog.sql` instead of delete-first seed files.
...
| `poke-sim/db/seed_teams_v2.sql` | Generated 27-team fresh-DB/reference seed; do not use delete-first seed files on live DBs with analysis history |
| `poke-sim/db/migrations/2026_06_20_align_shared_27_team_catalog.sql` | Generated non-destructive live DB seed alignment path; matches `poke-sim/data.js` catalog |
...
| M2 Backfill team seed | [POK-18](https://linear.app/poke-e-sim/issue/POK-18) | Current repo seed target is 27 teams; use non-destructive repair for live DBs |
...
2. **Seed repair / alignment (POK-18)** — run `db_m2_seed_tests.js` with `RUN_LIVE_DB=1` when credentials are available; current expected count is 27 teams.
