...
Implementation start: `v2.2.38-csp-xss-guard` adds `release_manifest.js` as the first source-of-truth layer for visible build ID, export `build_id`, service worker cache identity, source-sync policy, committed bundle SHA, and deployed GitHub Pages artifact SHA. `v2.2.38-csp-xss-guard` starts the public launch security gate by testing that local credentials stay ignored, committed app shells keep Supabase placeholders empty, Pages deploy injects only anon/public runtime config, and service-role/write/database secrets cannot land in browser runtime files. `v2.2.38-csp-xss-guard` adds a baseline Content Security Policy plus CI checks for dynamic code execution, inline event-handler attributes, Pages CSP preservation, and existing public XSS regression coverage. Because the current static bundle still needs inline script/style allowances, removing those allowances remains part of the UI/runtime split milestone rather than a hidden claim of full CSP lockdown.
