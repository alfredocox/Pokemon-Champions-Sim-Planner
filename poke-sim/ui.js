...
function detectTeamArchetypes(teamKey) {
  if (typeof TEAM_META !== 'undefined' && TEAM_META[teamKey] && TEAM_META[teamKey].archetypes)
    return TEAM_META[teamKey].archetypes;
  const team = TEAMS[teamKey];
  ...
  if (allMoves.filter(m => ['Earthquake','Rock Slide','Heat Wave','Eruption','Hyper Voice'].includes(m)).length >= 2)
    tags.add('aggressive');
  return tags.size > 0 ? [...tags] : ['balance'];
}

function getTeamGuide(teamKey) {
  if (typeof TEAM_META !== 'undefined' && TEAM_META[teamKey]) return TEAM_META[teamKey];
  return { archetypes: detectTeamArchetypes(teamKey), guide: 'Run simulations to generate pilot advice.', counters: [], setupPlays: [] };
}

function renderArchetypeTags(archetypes) {
  if (typeof TEAM_ARCHETYPES === 'undefined') return '';
...
function renderMoveLog(pokemonName) {
  if (typeof POKEMON_MOVE_LOG === 'undefined') return '';
...
function renderSetupPlays(teamKey) {
  if (typeof TEAM_META === 'undefined' || !TEAM_META[teamKey]) return '<em style="color:#888;font-size:0.78rem">Run simulations for setup play recommendations.</em>';
  const meta = TEAM_META[teamKey];
  if (!meta.setupPlays || !meta.setupPlays.length) return '<em style="color:#888;font-size:0.78rem">No setup plays defined.</em>';
  ...
  el.innerHTML = `
    <div style="padding:16px">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px">
        ${(team && team.members || []).filter(m => typeof POKEMON_MOVE_LOG !== 'undefined' && POKEMON_MOVE_LOG[m.name]).map(m => `
...
// Wire strategy tab button + player-select reactivity
(function patchStrategyTabButtons() {
  function wire() {
    ...
    // When player-select changes, live-update strategy if visible
    document.getElementById('player-select')?.addEventListener('change', (e) => {
      const panel = document.getElementById('tab-strategy');
      if (panel && panel.classList.contains('active')) renderStrategyTab(e.target.value);
    });
  }
