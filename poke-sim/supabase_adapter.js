...
(function () {
  ...
  async function loadTeamsFromDB() {
    ...
    try {
      ...
      const { data: members, error: mErr } = await sb
        .from('team_members').select('*').order('slot', { ascending: true });
      if (mErr) throw mErr;
      ...
      for (const m of members) {
        ...
        memberMap[m.team_id].push({
          name: m.species, item: m.item || '', ability: m.ability || '',
          nature: m.nature || '', level: m.level || 50, evs: m.evs || {},
          moves: m.moves || [], teraType: m.tera_type || '', role: m.role_tag || ''
        });
      }
      ...
      for (const t of teams) {
        result[t.team_id] = {
          ...
          source: t.source, members: memberMap[t.team_id] || []
        };
      }
  ...
  async function saveAnalysis(payload) {
    ...
    try {
      ...
      if (payload.win_conditions && payload.win_conditions.length) {
        const wcRows = payload.win_conditions.map(wc => ({ analysis_id, label: wc.label, count: wc.count }));
      ...
      if (payload.logs && payload.logs.length) {
        const logRows = payload.logs.slice(0, 50).map((l, i) => ({
          analysis_id, log_index: i, result: l.result || 'unknown',
          turns: l.turns || 0, tr_turns: l.tr_turns || 0,
          win_condition: l.win_condition || null, log: l.log || {}
        }));
  ...
  window.SupabaseAdapter = { enabled: ENABLED, loadTeamsFromDB, saveAnalysis, loadRecentAnalyses };
